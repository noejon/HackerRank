...
(function (window) {
    ...
    /* This was added to set the element to striked when an iem is completed */
    View.prototype._completeItem = function (id) {
        var label = qs('[data-id="' + id + '"] label');
        if(label) {
            label.className = 'striked';
        }
        var parentElement = qs('[data-id="' + id + '"] .view');
        var completeButtonToDelete = qs('[data-id="' + id + '"] .complete');
        if(completeButtonToDelete) {
            parentElement.removeChild(completeButtonToDelete);
        }
    };
